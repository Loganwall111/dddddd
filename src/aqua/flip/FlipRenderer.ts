...
// three 0.186 does not export the TSL Node type from the main namespace;
// the builders only use TSL expression nodes, so `any` is a faithful stand-in here.
...
export class FlipRenderer {
  ...
  render(): void {
    ...
    // The union renderer's setRenderTarget signatures are WebGL-specific in
    // three 0.186's types; structurally these targets are compatible at runtime.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
